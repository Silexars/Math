#include <Veritas/Math/Quaternion.h>
...
Quaternion::Quaternion(float32 a, float32 b, float32 c, float32 d) : a(a), b(b), c(c), d(d) { }
Quaternion::Quaternion(const Complex &ab, const Complex &cd) : a(ab.a), b(ab.b), c(cd.a), d(cd.b) { }
...
Quaternion Quaternion::lookAt(const vec3 &direction, const vec3 &up) {
    ...
    if (tr > 0) {
      float32 S = sqrt(tr+1.0f) * 2.0f; // S=4*qw
      qw = 0.25f * S;
      qx = (m21 - m12) / S;
      qy = (m02 - m20) / S;
      qz = (m10 - m01) / S;
    } else if ((m00 > m11)&(m00 > m22)) {
      float32 S = sqrt(1.0f + m00 - m11 - m22) * 2.0f; // S=4*qx
      qw = (m21 - m12) / S;
      qx = 0.25f * S;
      qy = (m01 + m10) / S;
      qz = (m02 + m20) / S;
    } else if (m11 > m22) {
      float32 S = sqrt(1.0f + m11 - m00 - m22) * 2.0f; // S=4*qy
      qw = (m02 - m20) / S;
      qx = (m01 + m10) / S;
      qy = 0.25f * S;
      qz = (m12 + m21) / S;
    } else {
      float32 S = sqrt(1.0f + m22 - m00 - m11) * 2.0f; // S=4*qz
      qw = (m10 - m01) / S;
      qx = (m02 + m20) / S;
      qy = (m12 + m21) / S;
      qz = 0.25f * S;
